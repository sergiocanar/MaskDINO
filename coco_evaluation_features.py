...
class COCOEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input_, output in zip(inputs, outputs):
            ...
            prediction = {"image_id": input_["image_id"]}
            
            global_info = global_info.detach().cpu().tolist()
            prediction['global_ft'] = global_info
    ...
    def _eval_predictions(self, predictions, img_ids=None):
        ...
        if hasattr(self._metadata, "thing_dataset_id_to_contiguous_id"):
            ...
            for result in coco_results:
                ...
                if result.get("global_ft") is not None:
                    continue
...
def instances_to_coco_json(instances, img_id, global_ft):
    """
...
def _evaluate_predictions_on_coco(
    ...
    if iou_type == "segm":
        ...
        for c in coco_results:
            c.pop("bbox", None)
    coco_results = [c for c in coco_results if 'global_ft' not in c]
    coco_dt = coco_gt.loadRes(coco_results)
    # breakpoint()
    dti = [r['category_id'] for r in coco_dt.dataset['annotations'] if r['image_id']==1351]
    print(dti)
