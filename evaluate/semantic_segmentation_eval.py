...
from .utils import (
    decode_rle_to_mask,
    decode_polygon_to_mask,
)  # Custom function to decode Run-Length Encoding (RLE) into masks
...
def eval_segmentation(task, coco_anns, preds, img_ann_dict, **kwargs):
    ...
    if "instruments_categories" in coco_anns:
        ...
        cats = (
            coco_anns[f"{task}_categories"]
            if f"{task}_categories" in coco_anns
            else coco_anns["categories"]
        )
    ...
    for image in tqdm(coco_anns, desc="Evaluating Semantic Segmentation"):
        ...
        if "mask_path" in kwargs and kwargs["mask_path"] is not None:
            ...
            gt_img = io.imread(
                os.path.join(mask_path, file_name.split(".")[0] + ".png")
            )
            ...
            for im_ann in image_anns:
                ann_cat = (
                    im_ann["instruments"]
                    if "instruments" in im_ann
                    else im_ann["category_id"]
                )
                gt_classes.add(ann_cat)
                if type(im_ann["segmentation"]) == list:
                    p_mask = decode_polygon_to_mask(
                        im_ann["segmentation"], width, height
                    )
                elif type(im_ann["segmentation"]) == dict:
                    p_mask = decode_rle_to_mask(im_ann["segmentation"], "uint8")
                else:
                    raise ValueError(
                        f"Unsupported segmentation type: {type(im_ann['segmentation'])}"
                    )
                all_masks.append(p_mask * (im_ann["category_id"]))
            if len(np.array(all_masks).shape) == 1:
