...
class PerImageEvaluation(object):
    ...
    def _compute_tp_fp(
        ...
        for i in range(self.num_groundtruth_classes):
            ...
            if (
                ...
                and detected_masks is not None
                and any(tp_fp_labels)
            ):
                scores_mask, tp_fp_labels_mask = (
                    self._compute_tp_fp_for_single_class_mask(
                        detected_boxes=detected_boxes_at_ith_class,
                        detected_scores=detected_scores_at_ith_class,
                        groundtruth_boxes=gt_boxes_at_ith_class,
                        groundtruth_is_difficult_list=groundtruth_is_difficult_list_at_ith_class,
                ...
                tp_fp_labels = np.logical_and(tp_fp_labels, tp_fp_labels_mask)

                assert all(scores_mask == scores), f"{scores_mask} {scores}"
    ...
    def _get_overlaps_and_scores_mask_mode(
        ...
        Returns:
          ...
          num_boxes: Number of non-maximum suppressed detected boxes.
        """
        detected_boxmasklist = np_box_mask_list.BoxMaskList(
            detected_boxes, detected_masks
        )
        detected_boxmasklist.add_field("scores", detected_scores)
        gt_non_group_of_boxmasklist = np_box_mask_list.BoxMaskList(
            groundtruth_boxes[~groundtruth_is_group_of_list],
            groundtruth_masks[~groundtruth_is_group_of_list],
        )
        iou = np_box_mask_list_ops.iou(
            detected_boxmasklist, gt_non_group_of_boxmasklist
        )
        scores = detected_boxmasklist.get_field("scores")
        num_boxes = detected_boxmasklist.num_boxes()
        return iou, None, scores, num_boxes

    def _compute_tp_fp_for_single_class_mask(
        self,
        detected_boxes,
        detected_scores,
        groundtruth_boxes,
        ...
        (
            ...
            scores,
            num_detected_boxes,
        ) = self._get_overlaps_and_scores_mask_mode(
            detected_boxes=detected_boxes,
            detected_scores=detected_scores,
            groundtruth_boxes=groundtruth_boxes,
            groundtruth_is_group_of_list=groundtruth_is_group_of_list,
