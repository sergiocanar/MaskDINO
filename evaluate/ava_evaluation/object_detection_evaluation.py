...
def transform_coded_masks_to_array(masks, height=800, width=1280):
    all_masks = []

    for mask in masks:
        if type(mask) == dict:
            all_masks.append(m.decode(mask))
        elif type(mask) == list:
            polys = [np.array(p).flatten().tolist() for p in mask]
...
class ObjectDetectionEvaluator(DetectionEvaluator):
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        self._evaluation.add_single_ground_truth_image_info(
            ...
            image_size=groundtruth_dict[standard_fields.InputDataFields.image_size],
        )
    ...
    def evaluate(self):
        ...
        for idx in range(per_class_ap.size):
            if idx + self._label_id_offset in category_index:
                ...
                PR_results[display_name.split("/")[-1]] = [
                    per_class_precisions[idx],
                    per_class_recalls[idx],
                ]
        ...
        return pascal_metrics, PR_results
...
class ObjectDetectionEvaluation(object):
    ...
    def add_single_ground_truth_image_info(
        ...
        self.image_sizes[image_key] = image_size
    ...
    def add_single_detected_image_info(
        ...
        if image_key in self.groundtruth_boxes:
            ...
            width, height = self.image_sizes[image_key]
            if groundtruth_masks is not None:
                groundtruth_masks = transform_coded_masks_to_array(
                    groundtruth_masks, height=height, width=width
                )
            if detected_masks is not None:
                detected_masks = transform_coded_masks_to_array(
                    detected_masks, height=height, width=width
                )
