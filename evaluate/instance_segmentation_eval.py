...
def eval_segmentation(task, coco_anns, preds, img_ann_dict, **kwargs):
    ...
    results, PR_results = run_evaluation(
...
def organize_pred_pascal(gt_keys, preds, task, num_classes):
    ...
    for new_key in tqdm(gt_keys):
        ...
        pred_image.sort(key=lambda x: x[f"{task}_score_dist"], reverse=True)
        ...
        pascal_evaluator.add_single_ground_truth_image_info(
            image_key,
            {
                ...
                standard_fields.InputDataFields.groundtruth_classes: np.array(
                    labels[image_key],
                    dtype=int,  # Tiene que haber mismo número de cajas que de labels
                ),
                ...
                standard_fields.InputDataFields.image_size: np.array(
                    image_sizes[image_key], dtype=int
                ),
            },
        )
