...
def gather_info(coco_json):
    ...
    for image in coco_json["images"]:
        ...
        if "video_name" not in image or "frame_num" not in image:
            # breakpoint()
            frame_name = image["file_name"].split("/")[0]
            case, frame = frame_name.split("_")
...
def filter_preds(preds_list, method, parameters):
    ...
    if "cls" in method:
        ...
        for pred in preds_list:
            if (
                (not use_thresh)
                or (use_thresh and pred["score"] >= thresh[pred["category_id"]])
            ) and (
                (not use_topk)
                or (
                    use_topk
                    and len(cls_preds[pred["category_id"] + 1])
                    < topk[pred["category_id"]]
                )
            ):
